...
class get_model(nn.Module):
    def __init__(self, num_class, need_speed = True, chanel_num = 3):
        ...
        if need_speed:
            ...
            self.conv1_speed = torch.nn.Conv1d(1, 64, 1)
            self.conv2_speed = torch.nn.Conv1d(64, 128, 1)
    ...
    def forward(self, x):
        ...
        x_speed_low = F.relu(self.bn1_speed(self.conv1_speed(x_speed))) # 速度特征 B * 64 * N
        x_speed_high = F.relu(self.bn2_speed(self.conv2_speed(x_speed_low))) # 速度特征 B * 128 * N
        x_speed = F.relu(self.conv3_speed(x_speed_high))# 速度pred( B T N )-- T 语义标签数
        
        
        x, trans, trans_feat, x_local = self.feat(x) # B * 1088 * N

        # 在高维空间融合速度特征
        x_low = torch.cat((x_local, x_speed_low), 1)
        x_low_prime = x_low[torch.randperm(x_low.size(0))] # B * 256 * N
        x_low_global = F.adaptive_max_pool1d(x_low, 1).view(batchsize, -1)
        x_low_global_prime = x_low_global[torch.randperm(x_low_global.size(0))] # B * 256 * N
        ...
        x_high_exp = x + x_speed_high # B * 128 * N
        x_high = F.adaptive_max_pool1d(x_high_exp, 1).view(batchsize, -1) # b * 128
        # 在低维空间做速度特征融合
        x = torch.cat((x, x_speed), 1) # 节点2 B * 136 * N 
        ...
        return x, trans_feat, x_low, x_low_prime, x_low_global, x_low_global_prime, x_high, x_high_exp
